...
const TRANSFORMER_RE = /#([a-zA-Z_][a-zA-Z0-9_\-]*)\(([^()]+)\)/g;
...
const DEFAULT_OPTIONS =
{
	media:
	{
        'xs'        : 'not all and (min-width: 576px)',
        'sm'        : '(min-width: 576px)',
        'md'        : '(min-width: 768px)',
        'lg'        : '(min-width: 992px)',
        'xl'        : '(min-width: 1200px)',
...
module.exports = class Style
{
    ...
    {
        return { ...scope, variables: { ...scope.variables }, generators: { ...scope.generators }, transformers: { ...scope.transformers }, ...extend }
    }
    ...
    {
        return value.replace( VARIABLE_RE, variable => scope.variables[ variable ]).replace( TRANSFORMER_RE, ( _, transformer, args ) => 
        {
            let literal = _;

            if( transformer = scope.transformers[ transformer ])
            {
                literal = transformer.literal;
                args = Object.fromEntries( args.trim().split(',').map(( arg, i ) => [ transformer.arguments[i].variable, arg.trim() ]));

                Object.entries( args ).forEach(([ name, value ]) =>
                {
                    literal = literal.replace( new RegExp( '\\$' + name.substring(1), 'g' ), value );
                });
            }
    ...
    static #resolve_style( scope, style, recursive = false )
    {
        for( let entry of style )
        {
            ...
            else if( entry.transformer )
            {
                scope.transformers[ entry.transformer.name ] = entry.transformer;
            }
            ...
            else if( entry.generate )
            {
                ...
                for( let i = 0; i < generator.arguments?.length; ++i )
                {
                    ...
                    args[ generator.arguments[i].variable ] = entry.generate.arguments[i] ? Style.#resolve_value( scope, entry.generate.arguments[i] ) : generator.arguments[i].default;
                }
    ...
    static compile( source, options = {})
    {
        ...
        let compiled = Style.#resolve_style({ options, selector: '', rule: '', variables: {}, generators: {}, transformers: {}, buffer: { properties: [], source: '' }}, style );
